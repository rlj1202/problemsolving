...
struct Cost { int cushion; int node; };
...
unsigned long long pathcount[902];
...
int main() {
	...
	pq.push(mincost[startnode] = {0, startnode});
	...
	while (!pq.empty()) {
		...
		for (int next : adj[cur.node]) {
			Cost newcost = {cur.cushion + nodes[next].added, next};
	...
	if (mincost[endnode].cushion == INF) {
		printf("%d\n", -1);
	} else {
		printf("%d\n", mincost[endnode].cushion);
		printf("%llu\n", pathcount[endnode]);
	}
